...
3. For static audit requests, ask for the target repo path or URL, initialize the output folder with `scripts/init_output.py`, then run `scripts/static_audit.py` internally when shell execution is available. **When a domain is named, always pass `--domain <domain>`** so `domain_extensions/<domain>/regime_overlay.json`, if present, is schema-validated and merged into that run's required controls — check the JSON output's `domain_overlay_loaded` field and say plainly whether an overlay was found, rather than assuming one was used. Write static artifacts under `02_static_audit/` unless the user asks for specific destinations. Use the script for framework gating, obvious control discovery, required-control derivation, and hashable finding scaffolding.
...
13. **When a live judge-model API key is unavailable, or the client's governance rules do not allow sending case transcripts to an externally-called judge model, offer a Judge Handoff Pack instead of live DeepEval judge execution.** Run `scripts/judge_handoff_export.py` internally against the normalized dataset and recorded results to write `06_evals/judge_handoff/{judge_rubric.md, judge_cases.jsonl, judge_result_template.json, judge_result_template.xlsx, instructions_for_client.md}`. The rubric is sourced from `engine/judge_rubric.py`, the same criteria a live DeepEval run would use, so results are comparable either way. **When a domain is named, pass `--domain <domain>` to both `scripts/judge_handoff_export.py` and `scripts/judge_result_import.py`** — omitting it silently generates a generic rubric even when a domain-specific `domain_extensions/<domain>/judge_rubric_overlay.md` exists (e.g. `financial_aml`'s tipping-off, SAR-approval, and analyst-usefulness dimensions), and the importer's recorded `rubric_criteria` provenance would then not match what the client actually scored against. Tell the user the deterministic hard-gate results in `06_evals/results/eval_summary.md` are already complete and do not depend on this pack — only the semantic-quality (`llm_judge_score`) layer is deferred. When the client returns a completed `judge_result.json`, run `scripts/judge_result_import.py` (with the same `--domain`) to produce the `llm_judge_score` summary; surface any `data_quality_issues` it reports (missing judge model attribution, missing run date, inconsistent pass/fail, unexpected case ids) rather than treating an unattributed score as equivalent to a fully attributed one.
14. Read `evals/eval_framework_adapter_pattern.md` before adding support for another eval framework such as Ragas, LangSmith, Promptfoo, Inspect AI, garak, or Phoenix.
15. Read `regimes/domain_extension.md` when the user asks to add domains, sub-business lines, quality profiles, domain knowledge, MDM/data expectations, or collaborator extension patterns.
16. Before rendering a final report, read `output/report_template.md`. Use `scripts/render_report.py` internally for Markdown, XLSX, or DOCX output. Write final reports under `07_reports/` unless the user asks otherwise. If report format is unspecified, ask whether the user wants Markdown, Excel, DOCX, or all; default to Markdown plus Excel when the user says "whatever is best".
17. After any meaningful run, use `scripts/render_stage_artifacts.py` to render human-readable Markdown for every stage that has evidence: intake, observation, static audit summary, gap matrix, change plan, dataset readiness, eval summary, remediation notes, and evidence summary. JSON and JSONL are evidence, not the primary user-facing output. **When the run used `--domain`, also run a second static audit of the same target without `--domain` (a quick baseline, not a separate reported run) and pass its JSON as `--baseline-audit-json` so `render_stage_artifacts.py` renders `03_gap_matrix/domain_overlay_diff.md`** — the concrete, requirement-by-requirement proof of what the domain pack changed. Cite this file when a user asks what a domain profile actually does.
18. If the user asks to fix or enable controls, read the matching remediation guide. For Google ADK, read `remediation/google_adk.md`. For OpenAI Agents SDK, read `remediation/openai_agents_sdk.md`. For LangGraph or LangChain, provide framework-specific guidance from the adapter docs and runner adapters.
19. Emit a verdict using `output/verdict_schema.md` and one hashable evidence file using `output/evidence_schema.md`.
...
- `engine/judge_rubric.py`: single source of truth for the LLM-as-judge criteria/threshold, consumed by both `scripts/deepeval_export.py` and `scripts/judge_handoff_export.py` so they can never score against different rubrics.
- `scripts/judge_handoff_export.py`: deterministic generator for a client-run Judge Handoff Pack (rubric, cases, result template, instructions) when no local judge-model API key is available or client governance forbids it; does not invoke an LLM.
- `scripts/judge_result_import.py`: deterministic importer for a client-returned judge result file; recomputes pass/fail from score vs. threshold and flags data-quality issues rather than trusting an unattributed score.
